...
void InitGameValues()
{
	...
	RealiseQData();
	archiev.RealiseQuest();
	archiev.RealiseNotes();
	archiev.RealiseContacts();

	archiev.AddContact("razlo");
...
void Game()
{
	...
	{
		OpenDataFile();
		Temp.LoadBMP(&Screen,NDXST_SYSMEM,MD.GetFile("MENU/ayes.bmp"),true);
		Temp.SetColorKey(Screen.PixelFormat->Rgb(255,0,255));
		Temp.SetTransP(true);
		MD.Close();
	...
	{
		// проиграем вступительный ролик новой игры
		PlayNGIntro();
	}
	...
	switch(wParam)
	{
		...
		case 0xC0: // ~
			if(mode==3)
			{
				...
				{
					if(cheat=="artmoney") // чит на бабосы
						plr_Inv.AddItem(1000, "money", &Screen);
					else if(cheat=="immortal") // чит на бессмертие
					{
						if(pFight->immortal)
							pFight->immortal=false;
						else
							pFight->immortal=true;
					}
					cheat="";
					cheats=false;
				}
	...
	/*		InitGameValues();
		...
		case 0x59: //Y
			if((mode==3)&&(cheats))
				cheat+="y";
			break;
		case 0x49: // I, Inventory
			if((mode==3)&&(cheats))
				cheat+="i";
			else if(GetData("Inventory") == 1)
			{
				if(mode==2)
				{
					Click();
...
//					OpenSStream("IPICKUP.wav");
		...
		else if((inv_mode==3)||(inv_mode==-4)) // окно экипировки
		{
			...
			else if((MouseX>=329)&&(MouseX<=467)&&(MouseY>=400)&&(MouseY<=461)) // слот оружия
			{
				...
				else if((MouseLBup)&&(drag==1)) // если перетаскиваем из слота оружия
				{
					drag=0;
					OpenSStream("IPUTDOWN.wav");
				}
	...
	else if(mode==5)
	{
		...
		if((drag==0)&&(MouseLB))
		{
			...
			if(plr_Inv.scrollable) // если нажали на кнопки прокрутки в инвентаре игрока
			{
			...
			if(obj_Bar.scrollable) // если нажали на кнопки прокрутки в столе обмена торговца
			{
			...
			if(plr_Bar.scrollable) // если нажали на кнопки прокрутки в столе обмена игрока
			{
...
void MainMenu()
{
	...
	if((Interface[0].iFrame<=256)&&(Interface[0].iFrame>=0))
		Temp.DrawTranslucent(Screen.Back, 0, 40, Interface[0].iFrame);
	else if(Interface[0].iFrame>256)
		Temp.DrawTranslucent(Screen.Back, 0, 40, 256);
...
//				PostMessage(hWindow, WM_CLOSE, 0, 0L);
		...
		{
			if(IEvent[1].Conditional==1) // new game
			{
				InitGameValues();
				Sleep(200);
				NDX_Surface			Fon; //фон
				...
				// проиграем вступительный ролик для новой игры

				#ifndef _DEBUG
				OpenDataFile();
				MD.ExtractFile_ToFile("VIDEO/ng_intro.wmv","temp");
				MD.Close();
				Video.OpenMedia(&Screen,"temp");
				Screen.Back->Clear(RGB(0,0,0));
				mode=-4;
...
//						PrepareFightFrm();
	...
	else if(mode==-2)
	{
		...
		else if(IEvent[1].Object==12) // кнопка отмены в экране загрузки / сохранения
		{
			if(IEvent[1].Conditional==1)
			{
				Sleep(200);
...
//	}
	...
	if(Notify.on)
	{
		...
		if(Notify.period>256)
			t=256;
		Messages.DrawTranslucent(Screen.Back, 608, 54, NDX_RECT(0,0+Notify.message*14,137,14+Notify.message*14), t);
	}
	...
	if (opened)
	{
		...
		else
		{
			for (UINT i=0; i<1000; ++i)
			{
				...
				ar >> Level[i].day;
				ar >> tmp;
				Crypt.TransformString(cryptkey, tmp);
				Level[i].location = tmp;
				ar >> tmp;
				Crypt.TransformString(cryptkey, tmp);
				Level[i].sublocation = tmp;
				ar >> Level[i].sublink;
				ar >> tmp;
				Crypt.TransformString(cryptkey, tmp);
				Level[i].text_data = tmp;
				ar >> tmp;
				Crypt.TransformString(cryptkey, tmp);
				Level[i].url1 = tmp;
				ar >> Level[i].url1_id;
...
//	setlocale( LC_ALL, "Russian.1251");
	...
	for(int i=lsp; i<space; ++i)
	{
		if((isupper(toascii(Level[frame].text_data.GetAt(i))) 
			...
			|| (Level[frame].text_data.GetAt(i) =='Ы')
...
//	allrows+=32;
	...
	// если во фрейме предусмотрено какое-либо событие, выполним его скрипт
	if((Level[frame].event!="")&&(last_mode!=3))
		CheckScript(Level[frame].event);
...
void PlayNGIntro()
{
	...
	// Start video if stopped
	Video.DrawMedia(Screen.Back,Scr2);			
	if (!Video.IsPlaying())
	{
		Video.Stop();
		Video.CloseMedia();
		...
		for(int i=17; i>0; i--)
		{
			Fon.DrawColorMix(Screen.Back, 0, 0, NDX_RECT(0, 0, 800, 600), Screen.PixelFormat->Rgb(255,255,255), 256-i*15);
			Screen.Flip();
			Sleep(60);
		}
...
void DrawHit()
{
	...
	OpenDataFile();
	Temp.LoadBMP(&Screen,NDXST_SYSMEM,MD.GetFile("MISC/blood.bmp"),true);
	Temp.SetColorKey(Screen.PixelFormat->Rgb(255,0,255));
	Temp.SetTransP(true);
	...
	if(Rand(0, 2)>0)
		Fon.DrawColorMix(Screen.Back, 0, 0, NDX_RECT(0, 0, 800, 600), /*Screen.PixelFormat->Rgb(255,0,0)*/RGB(255,0,0), /*230*/180);
	else
		Fon.DrawColorMix(Screen.Back, 0, 0, NDX_RECT(0, 0, 800, 600), /*Screen.PixelFormat->Rgb(255,0,0)*/RGB(255,0,0), /*230*/140);
	if(Rand(0,5)>1)
		Temp.DrawTranslucent(Screen.Back, 0, 0, 132);
	Screen.Flip();
...
void ShowInventory()
{
	...
	if((inv_mode==3)||(inv_mode==-4))
	{
		...
		if(pFight->Weapon[0].player_weapon!="")
		{
			...
			{
				scale = 60.0 / (double)pFight->Weapon[0].wpn_image.Height;
//				Screen.Back->FormattedText(333,324, RGB(207, 191, 63),fnt2, "%e", scale);
			}
			if(pFight->Weapon[0].wpn_image.Width>139)
			{
				double r = 137.0 / (double)pFight->Weapon[0].wpn_image.Width;
				if(r<scale)
					scale=r;
			}
...
void UseHaloDisk()
{
	archiev.AddHoloDisk(plr_Inv.GetDescription());
	plr_Inv.GetItem(1, plr_Inv.GetDescription(), &Screen);

	prog_error=true;
	error_msg="Данные диска загружены.";
	error_id=5;

	// сообщим игроку, что архивные записи обновились
	Notify.on = true;
	Notify.loop = 6;
	Notify.period = 0;
	Notify.message = 2;
...
void UseFlare()
{
	if((FindValue("turns_max"))&&(GetData("turns_max")!=0))
	{
		ModifyData("turns", (int)GetData("turns_max"));

		plr_Inv.GetItem(1, plr_Inv.GetDescription(), &Screen);
		plr_Inv.AddItem(1, "flare_on", &Screen);
	}
}
...
void LoadGame(CString fn)
{
	...
	CString temp;
	int tmp;
	int key; // ключ для дешифровки числовых значений
	...
	if (opened)
	{
		...
		ar >> temp;
		ar >> fr;
		ar >> key; // загрузим ключ для дешифровки
		ar >> tmp;
		...
		for (UINT i=0; i<tmp; ++i)
		{
			...
			pD->value = r / key / 513;
			ar >> temp;
			Crypt.TransformString(cryptkey, temp);
			pD->vname = temp;
		...
		for (i=0; i<tmp; ++i)
		{
			...
			pQuest->qtitle = temp;
			ar >> r;
			pQuest->qstatus = r / key / 513;

			archiev.Quests.Add(pQuest);
		}
		...
		for (i=0; i<tmp; ++i)
		{
			...
			plr_Inv.AddItem(amount, temp, &Screen);
		}

		// load player combat char-s
		int r;
		ar >> r;
		pFight->player_SDMGfrom = r / key / 513;
		ar >> r;
		pFight->player_SDMGplus = r / key / 513;
		ar >> r;
		pFight->player_SDMGto = r / key / 513;

		ar >> temp;
		Crypt.TransformString(cryptkey, temp);
		pFight->LoadWeapon(temp, &Screen);

		// загрузим значение таблицы
		for(i=0; i<10; ++i)
		{
			for(int j=0; j<10; ++j)
			{
...
void SaveGame(CString fn)
{
	...
	int key = t.GetMinute(); // используем как ключ для шифровки цифровых значений
	...
	if (opened)
	{
		...
		ar << r;
		ar << fr;
		ar << key; // сохраним наш ключ для дальнейшей дешифровки
		ar << QData.GetSize();
		...
		for (i=0; i<QData.GetSize(); ++i)
		{
			...
			ar << pD->value*key*513;
			time = pD->vname;
			Crypt.TransformString(cryptkey, time);
			ar << time;
		}

		ar << archiev.Quests.GetSize();
		...
		for (i=0; i<archiev.Quests.GetSize(); ++i)
		{
			CQuest* pQuest = (CQuest*) archiev.Quests.GetAt(i);
			time = pQuest->qtitle;
			Crypt.TransformString(cryptkey, time);
			ar << time;
			ar << pQuest->qstatus*key*513;
		}

		// структура данных заметок
		struct CNote
		{
			CString nNoteName; // имя заметки
		...
		ar << archiev.Notes.GetSize();

		for(i=0; i<archiev.Notes.GetSize(); ++i)
		{
			CNote* pNote = (CNote*) archiev.Notes.GetAt(i);
			time = pNote->nNoteName;
			Crypt.TransformString(cryptkey, time);
			ar << time;
		}

		ar << archiev.Contacts.GetSize();

		struct CContact
		{
			CString cContactName; // имя файла контакта
			CString cName; // имя
			CString cText; // содержание контакта
			bool cState; // раскрыт контакт или нет при просмотре
		};

		for(i=0; i<archiev.Contacts.GetSize(); ++i)
		{
			CContact* pContact = (CContact*) archiev.Contacts.GetAt(i);
			time = pContact->cContactName;
			Crypt.TransformString(cryptkey, time);
			ar << time;
		}
		...
		struct CHoloDisk
		{
			CString hDiskName; // имя файла голодиска
		...
		ar << archiev.HoloDisks.GetSize();

		for(i=0; i<archiev.HoloDisks.GetSize(); ++i)
		{
			CHoloDisk* pHoloDisk = (CHoloDisk*) archiev.HoloDisks.GetAt(i);
			time = pHoloDisk->hDiskName;
			Crypt.TransformString(cryptkey, time);
			ar << time;
			ar << pHoloDisk->hState;
		}
		...
		{
			ar << plr_Inv.GetAmount(i)*key*513;
			time = plr_Inv.GetDescription(i);
			Crypt.TransformString(cryptkey, time);
			ar << time;
		}

		// save player combat char-s
		ar << pFight->player_SDMGfrom*key*513;
		ar << pFight->player_SDMGplus*key*513;
		ar << pFight->player_SDMGto*key*513;

		time = pFight->Weapon[0].player_weapon;
		Crypt.TransformString(cryptkey, time);
		ar << time;

		// сохраним текущее значение таблицы
		for(i=0; i<10; ++i)
		{
			for(int j=0; j<10; ++j)
			{
...
void OnTimer()
{
	if(Window.Active)
	{
		...
		else if(mode==1)
		{
			if(anim==0)
			{
				if(Interface[0].iFrame <= /*248*/496)
					Interface[0].iFrame+=8;
				else
					anim=1;
			}
			else
			{
				if(Interface[0].iFrame >= -248)
					Interface[0].iFrame-=8;
				else
					anim=0;
			}
...
void InitScriptSystem()
{
	...
	else
	{
		...
		lua_register(luaVM, "AddContact", s_AddContact);
		lua_register(luaVM, "Play3DSound", s_Play3DSound);
		lua_register(luaVM, "Play3DSoundLoop", s_Play3DSoundLoop);
		lua_register(luaVM, "Stop3DSoundLoop", s_Stop3DSoundLoop);
		lua_register(luaVM, "PlaySound", s_PlaySound);
...
{
	archiev.AddNote(lua_tostring(luaVM, -1)); // добавим в архив заметку о воспоминании
...
int s_AddQuest(lua_State* luaVM)
{
	...
	if(!archiev.IsQuestPresent(lua_tostring(luaVM, i), lua_tonumber(luaVM, i-1)))
	{
		Notify.on = true;
		Notify.loop = 6;
		Notify.period = 0;
	...
	archiev.AddQuest(lua_tostring(luaVM, i), lua_tonumber(luaVM, i-1));
...
{
	archiev.AddContact(lua_tostring(luaVM, -1));
...
int s_Play3DSoundLoop(lua_State* luaVM)
{
	...
	Open3DSSoundLoop(lua_tostring(luaVM, -4), pos);
...
{
	OpenSStream(lua_tostring(luaVM, -1));
